...
class SRT_SOCKOPT(IntEnum):
    ...
    SRTO_IPTOS = (
        30  # IP Type of Service (passthru for system sockopt IPPROTO_IP/IP_TOS)
    )
    ...
    SRTO_CONNTIMEO = (
        36  # Connect timeout in msec. Caller default: 3000 rendezvous (x 10)
    )
    ...
    SRTO_MESSAGEAPI = (
        48  # In File mode use message API (portions of data with boundaries)
    )
    SRTO_PAYLOADSIZE = (
        49  # Maximum payload size sent in one UDP packet (0 if unlimited)
    )
    SRTO_TRANSTYPE = (
    ...
    SRTO_GROUPCONNECT = (
        57  # Set on a listener to allow group connection (ENABLE_BONDING)
    )
